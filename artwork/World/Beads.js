...
export default class Beads {
  ...
  setGeometry(beadScale, beadType) {
    ...
    } else if (beadType === data.beadType.icosahedron) {
      geometry = new IcosahedronGeometry(beadScale);
    } else {
  ...
  createMesh(model) {
    ...
    const reduceVertices = (array) => {
      return array.length < 10000
        ? array
        : reduceVertices(array.filter((element, i) => i % 2 === 0));
    };
    ...
    for (let i = 0; i < vertices.length; i++) {
      this.animateLoad
        ? this.mesh.setMatrixAt(
            i,
            matrix.setPosition(vertices[i][0], lowestStart, vertices[i][2])
          )
  ...
  #animateEvent() {
    ...
    this.animateButton.style.background = this.animateLoad
      ? data.colors.activeButton
      : data.colors.inactiveButton;
    this.animateButton.style.color = this.animateLoad
      ? data.colors.activeText
      : data.colors.inactiveText;
    this.animateButton.innerHTML = this.animateLoad
      ? data.buttons.animateOn
      : data.buttons.animateOff;
    this.#handleMeshEvent();
    this.resetMesh();
    this.createMesh(this.object);
  }
  ...
  #resetMeshEvent() {
    ...
    this.resetMeshButton.removeEventListener(
      "click",
      this.resetMeshEventHandler
    );
  ...
  #changeColorEvent() {
    ...
    this.colorButton.style.background = this.multiColored
      ? data.colors.activeButton
      : data.colors.inactiveButton;
    this.colorButton.style.color = this.multiColored
      ? data.colors.activeText
      : data.colors.inactiveText;
    this.colorButton.innerHTML = this.multiColored
  ...
  setEventListeners() {
    ...
    this.fileUploadButton.addEventListener(
      "change",
      (e) => e.target.files && this.loadFiles(e.target.files)
    );
  }
  ...
  update() {
    ...
    if (this.animateLoad && this.mesh && this.index < this.vertices.length) {
      this.mesh.setMatrixAt(
        this.index,
        matrix.setPosition(...this.vertices[this.index])
      );
